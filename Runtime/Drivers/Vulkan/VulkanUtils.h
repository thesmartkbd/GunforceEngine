...
#define ERROR_FAIL_V_MMAP(err) std::data(VulkanUtils::_GetVKErrorMessage(err))
#define DESCRIPTOR_TYPE_V_MMAP(type) std::data(VulkanUtils::_GetVkDescriptorTypeMessage(type))
#define PHYSICAL_DEVICE_TYPE_V_MMAP(type) std::data(VulkanUtils::_GetVkPhysicalDeviceTypeMessage(type))
...
namespace VulkanUtils
{
    ...
    static void LoadShaderModule(const char *filename, VkDevice device, VkShaderModule *pShaderModule)
    {
        ...
        LOGGER_WRITE_DEBUG("VulkanUtils::LoaderShaderModule: filename=%s, device=0x%p", filename, device);
        ...
        VkResult err;
        err = vkCreateShaderModule(device, &shaderModuleCreateInfo, VulkanUtils::Allocator, pShaderModule);
        LOGGER_WRITE_DEBUG("Create shader module(VkShaderModule), filename: %s, device: 0x%p, err: %s", filename, device, ERROR_FAIL_V_MMAP(err));
        IOUtils::Free(buf);
    }
